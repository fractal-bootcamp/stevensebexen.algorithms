...
export function iterationSteps<T>(fn: (x: T) => IterationStepResult<T>, inputValue: T, steps: IterationSteps<T> = [], depth = 10000): IterationSteps<T> {
...
export function iteration<T>(fn: (x: T) => IterationStepResult<T>, depth = 10000): (x: T) => IterationResult<T> {
  return (x: T) => {
